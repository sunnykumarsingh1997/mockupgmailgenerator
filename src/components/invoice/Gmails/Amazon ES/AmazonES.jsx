...
export default function AmazonES() {
  ...
  const handleFillAll = (jsonString) => {
    try {
      ...
      setName(data.name || "Juan Pérez");
      setEmail(data.email || "juan.perez@example.com");
      setOrderNo(data.tra1 || "402-1234567-1234567");
      setDay(new Date().toLocaleDateString('es-ES', { weekday: 'long' }));
      setDate(data.date || "1 de enero de 2024");
      setTime("10:00");
      setAmount(data.amount1 || "100,00");
      setItem(data.item || "Auriculares inalámbricos");
      setQTY(data.qty || "1");
      setAsin(data.asin || "B08XXXXXXX");
      setReason(data.reason || "Artículo defectuoso");
      setCard(data.card ? "Visa" : "MasterCard");
      setCardLast(data.card ? data.card.slice(-4) : "1234");
      setDownloadDate(new Date().toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' }));
    } catch (e) {
  ...
  return <>
    ...
    <Preview title="PREVIEW AMAZON ES FORM" />
    ...
    <div className="amazonDE-gmail-form" ref={invoiceRef}>
      ...
      <div className="amazon-mail-inner-content">
        ...
        <div class="amazon-gmail-from-to flex ">
          <p>
            <b>Amazon.es</b> &lt;payments-messages@amazon.es&gt;
          </p>
        ...
        <div className="amazon-gmail-content">
          ...
          Greetings from Amazon.es.<div className="amazon-spacer" />
          ...
          * to your {card} Credit Card ending with {cardLast}]: {formatWithCommas(amount)} INR
          ...
          <div className="amazon-spacer" />
          <p className="amazonDE-blue">http://www.amazon.es/gp/help/customer/display.html?nodeId=1161010</p>
          <div className="amazon-spacer" />
          Thank you for shopping at Amazon.es.
          <div className="amazon-spacer" />
          Sincerely,
          <div className="amazon-spacer" />
          Amazon.es Customer Service <br />
          <p className="amazonDE-blue">http://www.amazon.es</p>
        </div>
      </div>
